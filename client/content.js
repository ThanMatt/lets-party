...
socket.on('whoami', function ({ id, autogeneratedUsername }) {
  // console.log('myid', id);
  myid = id
  ...
  const extractedCode = currentUrl.match(/#[A-Za-z0-9\-_]+/g)

  console.log('Extracted code: ', extractedCode)
  if (!extractedCode?.length) return
  username = autogeneratedUsername
  // localStorage.setItem('lets_party_uname', nameinput.value)
  const roomCode = extractedCode[0].replace('#', '')
...
socket.on('joinmetothisroomsuccess', (payload) => {
...
function sendMessage(value) {
  if (value !== '') {
    ...
    status.innerHTML += `${username} says: ${value} </br>`
